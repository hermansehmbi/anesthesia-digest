...
def build_digest_email(articles: list, podcasts: list, bonus_podcasts: list,
                       ...
                       audio_filename: str | None = None) -> tuple[str, str]:
    ...
    # Audio summary card (API mode only). Email clients can't play media
    # inline, so this presents the summary nicely and points to the attached
    # MP3 (which has a descriptive, dated filename).
    audio_banner = ""
    if has_audio:
        fname = audio_filename or "audio summary"
        audio_banner = f"""
    <tr><td style="padding:18px 20px 6px;">
      <table width="100%" cellpadding="0" cellspacing="0" style="background:#0e2a3b;border-radius:10px;overflow:hidden;">
        <tr>
          <td width="76" valign="top" style="padding:20px 0 20px 20px;">
            <div style="width:48px;height:48px;border-radius:50%;background:#2e86c1;text-align:center;color:#ffffff;font-size:20px;line-height:48px;">&#9654;</div>
          </td>
          <td style="padding:18px 20px;">
            <div style="font-size:15px;font-weight:bold;color:#ffffff;font-family:Georgia,serif;">&#127911; Anesthesia Digest Audio Summary — {date_str}</div>
            <div style="font-size:12px;color:#cfe3f2;margin-top:5px;line-height:1.4;">A ~7 minute AI-generated audio overview of today's journal highlights.</div>
            <div style="font-size:11px;color:#9bb9d1;margin-top:10px;line-height:1.4;">&#9654; Attached as <strong style="color:#cfe3f2;">{fname}</strong> — open the attachment to listen.</div>
            <div style="font-size:10px;color:#7fa3bd;margin-top:10px;font-style:italic;line-height:1.4;">This audio summary was generated by AI (Claude + Edge TTS) and is not affiliated with any journal.</div>
          </td>
        </tr>
      </table>
    </td></tr>"""
